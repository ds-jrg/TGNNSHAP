...
def _set_tgat_data(lists,all_events: DataFrame, target_event_idx: Union[int, List]):
    """ supporter for tgat """
    ...
    #     target_t = all_events.iloc[target_event_idx-1, 2]
        target_u = int(lists[target_event_idx][0])
        target_i = int(lists[target_event_idx][1])
        target_t = int(lists[target_event_idx][2])
