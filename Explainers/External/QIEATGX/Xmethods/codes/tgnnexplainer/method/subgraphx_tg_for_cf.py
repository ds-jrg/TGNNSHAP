...
from Xmethods.codes.tgnnexplainer.method.base_explainer_tg_for_tgnn import BaseExplainerTG
from Xmethods.codes.tgnnexplainer.method.other_baselines_tg import _create_explainer_input
from Xmethods.codes.tgnnexplainer.method.tg_score import TGNNRewardWraper
...
class MCTS(object):
    ...
    def __init__(self, events: DataFrame, candidate_events = None, base_events = None, candidate_initial_weights = None,
                 ...
                 score_func: Callable = None, existed = 0, factual = 0, max_candidates = 0, max_nodes = 0,
                #  device='cpu'
                 ):
    ...
    def mcts_rollout(self, tree_node): # 
        ...
        # 使えるイベントが無くなった時
        if self.factual:
            if len( tree_node.coalition ) == len(self.candidate_events):
                return tree_node.P
        else:    
        ...
        if self.factual: # 追加，削除できる最大数の指定
            tree_coalition = len(self.candidate_events)-len(tree_node.coalition)
        else:
            # 追加していく方
            tree_coalition = len(tree_node.coalition)



        # if len(tree_node.children) != len(tree_node.coalition): #問題はここ
        if len(tree_node.children) != tree_coalition: #問題はここ
            # expand_events = tree_node.coalition
            # 事実の時は追加しているイベント以外を見たい，反事実は削除してないイベントを見たい
            #exist_childrenは事実では追加イベント，反事実では削除イベント

            if self.factual: # 使っていないイベントを候補としたい
                # ここで使っているイベントを取得
                use_events = set(tree_node.coalition)
                # childrenとして使っているイベントを取得
                exist_children = set(map( lambda x: x.created_by_remove, tree_node.children ))
                # 使っていないイベントを取得
                not_exist_children = list(filter(lambda e_idx: e_idx not in use_events and e_idx not in exist_children, self.candidate_events ) )
            else:# 使っているイベント以外を候補としたい
                exist_children = set(map( lambda x: x.created_by_remove, tree_node.children ))
                not_exist_children = list(filter(lambda e_idx:e_idx not in exist_children, tree_node.coalition ) )
            
            
            # ここで削除する順番を決めている(事実，反事実，存在，非存在の拡張もできた)
            expand_events = self._select_expand_candidates(not_exist_children,self.existed,self.factual) 
            ...
            for event in expand_events:
                if self.factual:
                    # 対象とするイベントの中から一つを取り除いたcoalitionを作成
                    important_events =  tree_node.coalition + [event,]
                    if len(important_events) > self.max_candidates: # NOTE 正しく動いてるか確認する
                        return tree_node.P # its score
                else:
                    # 対象とするイベントの中から一つを取り除いたcoalitionを作成
                    important_events = [e_idx for e_idx in tree_node.coalition if e_idx != event ]
                    if len(important_events) < self.max_candidates: # maxは使わないといけないイベントの数
                        return tree_node.P # its score
                # check the state map and merge the same sub-tg-graph (node in the tree)
                # impirtant_eventsが正しいか確認する
                find_same = False
            ...
            rewards = compute_scores(self.score_func, self.base_events, tree_node.children, self.event_idx)
            # import ipdb; ipdb.set_trace()
            for child, score in zip(tree_node.children, rewards):
                child.P = score
        ...
        if (self.existed & self.factual) or ((self.existed==0) & (self.factual==0)):
    ...
    def _select_expand_candidates(self, not_exist_children,existed,factual):
        assert self.candidate_initial_weights is not None
        # ここを変更する
        if (existed & factual) or ((existed==0) & (factual==0)):
            return sorted(not_exist_children, key=self.candidate_initial_weights.get)
        else:
            return sorted(not_exist_children, key=self.candidate_initial_weights.get, reverse=True) # descending
        # return sorted(not_exist_children, key=self.candidate_initial_weights.get)
        # return sorted(not_exist_children, key=self.candidate_initial_weights.get, reverse=True) # descending
    ...
    def _compute_node_score(self, node, sum_count):
        ...
        max_event_idx = max(self.candidate_events)
        # max_event_idx = max(self.root.node_coalition)
    ...
    def mcts(self, verbose=True):
        ...
        end_rollout = 0
        result_list = []
        num_2hop = len(self.candidate_events)
        for rollout_idx in pbar: #ここは最大500回
            ...
            get_best_score = curr_best_node.P
            loop_result.append([get_best_score,rollout_idx+1,0,num_2hop,str(curr_best_node.coalition)])
            if len(self.state_map) > self.n_rollout:
                end_rollout = rollout_idx

        end_time = time.time()
        self.run_time = end_time - start_time
        
        for node in self.state_map.values():
            if self.factual:
                result_list.append([node.P,len(node.coalition),0,num_2hop])
            else:
                result_list.append([node.P,num_2hop-len(node.coalition),0,num_2hop])


        tree_nodes = list(self.state_map.values())

        return tree_nodes, end_rollout,result_list,loop_result
    ...
    def _initialize_tree(self):
        ...
        if self.factual:
            self.root_coalition = []
            self.root = MCTSNode(self.root_coalition, created_by_remove=-1, c_puct=self.c_puct, Sparsity=0.0)
        else:
            self.root_coalition = copy.copy( self.candidate_events )
...
class SubgraphXTG(BaseExplainerTG):
    ...
    def __init__(self,sparsity, model, model_name: str, explainer_name: str, dataset_name: str, all_events: DataFrame,  explanation_level: str, device, 
                ...
                pg_explainer_model=None, pg_positive=True, factual=0,existed=0,
                ):

        super(SubgraphXTG, self).__init__(sparsity=sparsity,
                                          model=model, 
    ...
    def explain(self,
        ...
        if self.explanation_level == 'node':
            ...
            self.mcts_state_map = MCTS(events=self.ori_subgraph_df,
                                       candidate_events=self.candidate_events, # all 2hop events
                                       base_events=self.base_events, # base events are not none
                                       node_idx=node_idx, 
                                       event_idx=event_idx,
            ...
            tree_nodes,end_rollout,result_list,loop_result = self.mcts_state_map.mcts(verbose=self.verbose) # search ここで実行
        ...
        return tree_nodes, tree_node_x,end_rollout,result_list,loop_result
    ...
    def __call__(self, all_events = None,node_idxs: Union[int, None] = None, event_idxs: Union[int, None] = None,sparsity: Union[float, None] = None, return_dict=None, device=None):
        """
        Args:
        ...
        results_list = []
        for i, event_idx in enumerate(event_idxs): # ループじゃなくて１回だけ実行
            # print(f'\nexplain {i}-th: {event_idx}')
            # construct TGNN reward function
            # これで存在していないイベントのrewardの計算を可能にする
            if factual == 0:
                self.tgnn_reward_wraper = TGNNRewardWraper(self.model, self.model_name, self.all_events, self.explanation_level)

            self._initialize(event_idx) # sparsityに合わせてcandidate_eventsを選ぶ
            
            num_candidates = max(1,int(len(self.candidate_events)*sparsity))
            if factual == 0:
                num_candidates = len(self.candidate_events) - num_candidates
            self.max_candidates = num_candidates # NOTE: max_candidatesはあってるか？(fac,cfの両方)
            # sorted_list = sorted(self.candidate_events, reverse=True) # 上位から選ぶ
            # top_N = sorted_list[:num_candidates]
            # # print(top_N)
            # self.candidate_events = top_N

            if self.load_results: # if use saved results
                AssertionError('Not implemented yet')
                tree_nodes, tree_node_x = self._load_saved_nodes_info(event_idx)
            else:
                tree_nodes, tree_node_x,end_rollout,result_list,loop_result = self.explain(event_idx=event_idx)
            ...
            result = [tree_nodes, tree_node_x,self.mcts_state_map.recorder]
            results_list.append(result)
            
            if return_dict is not None:
                return_dict[event_idx] = result

        return results_list,end_rollout,result_list,loop_result
        # return tree_nodes, tree_node_x
