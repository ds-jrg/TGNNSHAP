...
from Xmethods.codes.tgnnexplainer.method.base_explainer_tg import BaseExplainerTG
from Xmethods.codes.tgnnexplainer.evaluation.metrics_tg_utils import fidelity_inv_tg
...
def _create_explainer_input(model: Union[TGAN, TGN], model_name, all_events, candidate_events=None, event_idx=None, device=None):
    ...
    if model_name in ['tgat', 'tgn']:
        ...
        # candidate_t_emb = model.time_encoder(torch.tensor(candidate_events_t, dtype=torch.float32, device=device).view(1, -1)).view(len(candidate_events_t), -1)
...
class PGExplainerExt(BaseExplainerTG):
    ...
    def __call__(self, node_idxs: Union[int, None] = None, event_idxs: Union[int, None] = None, tgt_src: Union[int, None] = None, tgt_dst: Union[int, None] = None, sparsity: Union[float, None] = None,factual: Union[bool, None] = None,existed: Union[bool, None] = None):
        ...
        if not self.explainer_ckpt_path.exists():
            AssertionError(f'ckpt not found at {str(self.explainer_ckpt_path)}')
            # self._train() # we need to train the explainer first
        else:
            state_dict = torch.load(self.explainer_ckpt_path, map_location=self.device, weights_only=True)
        ...
        for i, event_idx in enumerate(event_idxs):
            ...
            events_save = events.copy()
            events.iloc[event_idx-1,0] = tgt_src
            events.iloc[event_idx-1,1] = tgt_dst
            # print(tgt_src,tgt_dst,events.iloc[event_idx-1,2])
            ...
            self.all_events = events_save

        return results_list, candidates
    ...
    def explain(self, node_idx=None, event_idx=None):
        ...
        # ここで、イベントを変更したい
        # print(self.all_events.iloc[event_idx-1])
        input_expl = _create_explainer_input(self.model, self.model_name, self.all_events, \
            candidate_events=self.candidate_events, event_idx=event_idx, device=self.device)
