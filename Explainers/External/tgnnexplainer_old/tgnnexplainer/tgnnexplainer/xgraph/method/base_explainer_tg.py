...
from ..dataset.utils_dataset import k_hop_temporal_subgraph
...
class BaseExplainerTG(object):
    def __init__(
        ...
        self.tgnn_reward_wraper = TGNNRewardWraper(
            self.model, self.neighbor_finder, self.model_name, self.all_events, self.explanation_level
        )
    ...
    def find_candidates(self, target_event_idx):
        ...
        if self.model_name in ["tgat", "tgn"]:
            ...
            edge = self.all_events[self.all_events.idx == target_event_idx]

            u = edge.u.values[0]
            i = edge.i.values[0]
            ts = edge.ts.values[0]

            _, edges, _ = self.neighbor_finder.get_multi_hop_neighbors(self.num_layers, np.array([u,i]), np.array([ts, ts]), num_neighbors = self.num_neighbors)

            accu_e_idx = np.concat(edges, axis=1)
            unique_e_idx = np.unique(accu_e_idx)
            ...
            #         ngh_finder.get_historical_neighbors(
            #             last_nodes,
            #             last_ts,
            #             num_neighbors=num_neighbors,
            #             #kept_edge_ids=edge_idx_preserve_list,  # NOTE: not needed?
            #         )
            #     )
    ...
    def _set_candidate_events(self, event_idx):
        ...
        self.base_events = list(
            filter(lambda x: x not in candidate_events_set_, unique_e_idx)
        )  # NOTE: an importanct change, need test. largely influence the speed!
    ...
    def _set_tgnn_wraper(self, event_idx):
        ...
        self.tgnn_reward_wraper.compute_original_score(
            np.concat([self.base_events, self.candidate_events]), event_idx
        )
    ...
    @staticmethod
    def _score_path(
        results_dir, model_name, dataset_name, explainer_name, event_idx, seed
    ):
        """
        ...
        score_filename = (
            results_dir
            / f"{model_name}_{dataset_name}_{explainer_name}_{seed}_{event_idx}_candidate_scores.csv"
        )
