...
class SubgraphXTG(BaseExplainerTG):
    ...
    def __init__(self, model: TGNN, neighbor_finder: NeighborSampler, model_name: str, explainer_name: str, dataset_name: str, seed:int, all_events: DataFrame,  explanation_level: str, device, 
                ...
                pg_explainer_model=None, pg_positive=True, num_layers = 2, num_neighbors = 20
                ):
    ...
    def _set_candidate_weights(self, event_idx):
        ...
        input_expl = _create_explainer_input(self.model, self.model_name, self.neighbor_finder, self.all_events, \
        ...
        candidate_initial_weights = { candidate_weights_dict['candidate_events'][i]: candidate_weights_dict['edge_weights'][i] for i in range(len(candidate_events)) }
        self.candidate_initial_weights = candidate_initial_weights
