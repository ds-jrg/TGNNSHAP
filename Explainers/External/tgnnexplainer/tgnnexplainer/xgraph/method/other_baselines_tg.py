...
class PGExplainerExt(BaseExplainerTG):
    ...
    def _create_explainer(model, model_name, device):
        if model_name == 'tgat':
            expl_input_dim = getattr(model, 'explainer_input_dim', model.model_dim * 8)
        elif model_name == 'tgn':
    ...
    def _obtain_train_idxs(self,):
        ...
        if self.dataset_name in ['wikipedia', 'reddit']:
            ...
            train_e_idxs = np.random.choice(pos_events, size=min(size, len(pos_events)), replace=False)
        else:
            # DyGLib datasets use their own names (for example Generated).
            # Keep the paper's middle-of-stream sampling protocol when no
            # dataset-specific branch exists.
            low = max(1, int(len(self.all_events) * 0.2))
            high = max(low + 1, int(len(self.all_events) * 0.6))
            train_e_idxs = np.random.randint(low, high, size=size)
    ...
    def _train(self,):
        ...
        for e in range(self.train_epochs):
            ...
            loss_value = float(np.mean(loss_list)) if loss_list else 0.0
            tqdm.write(f"epoch {e} loss epoch {loss_value}, skipped: {skipped_num}, ckpt saved: {self.explainer_ckpt_path}")
