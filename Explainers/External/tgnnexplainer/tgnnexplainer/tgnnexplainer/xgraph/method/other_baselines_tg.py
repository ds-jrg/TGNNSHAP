...
from .base_explainer_tg import BaseExplainerTG
...
def _create_explainer_input(
    ...
    if model_name in ["tgat", "tgn"]:
        ...
        t_idx_u_emb = model.backbone.node_features[
            torch.tensor(event_idx_u, dtype=torch.int64, device=device), :
        ]
        t_idx_i_emb = model.backbone.node_features[
            torch.tensor(event_idx_i, dtype=torch.int64, device=device), :
        ]
        # import ipdb; ipdb.set_trace()
        t_idx_t_emb = model.backbone.time_encoder(
            torch.tensor(event_idx_t, dtype=torch.float32, device=device).reshape(
                (1, -1)
            )
        ).reshape((1, -1))
        t_idx_e_emb = neighbor_finder.edge_features[
            torch.tensor(
                [
                    event_idx_new,
                ],
                dtype=torch.int64,
                device=neighbor_finder.edge_features.device,
            ),
            :,
        ].to(device)

        target_event_emb = torch.cat(
            [t_idx_u_emb, t_idx_i_emb, t_idx_t_emb, t_idx_e_emb], dim=1
        )

        candidate_events_u, candidate_events_i, candidate_events_t = _set_tgat_data(
            all_events, candidate_events
        )
        candidate_events_new = candidate_events

        candidate_u_emb = model.backbone.node_features[
            torch.tensor(candidate_events_u, dtype=torch.int64, device=device), :
        ]
        candidate_i_emb = model.backbone.node_features[
            torch.tensor(candidate_events_i, dtype=torch.int64, device=device), :
        ]
        candidate_t_emb = model.backbone.time_encoder(
            torch.tensor(
                candidate_events_t, dtype=torch.float32, device=device
            ).reshape((1, -1))
        ).reshape((len(candidate_events_t), -1))
        candidate_e_emb = neighbor_finder.edge_features[
            torch.tensor(candidate_events_new, dtype=torch.int64, device=neighbor_finder.edge_features.device,), :
        ].to(device)

        candiadte_events_emb = torch.cat(
            [candidate_u_emb, candidate_i_emb, candidate_t_emb, candidate_e_emb], dim=1
        )

        input_expl = torch.cat(
            [
                target_event_emb.repeat(candiadte_events_emb.shape[0], 1),
                candiadte_events_emb,
            ],
            dim=1,
        )
        # import ipdb; ipdb.set_trace()
...
class PGExplainerExt(BaseExplainerTG):
    ...
    @staticmethod
    def _create_explainer(model: TGNN, device):
        expl_input_dim = 2 * (model.backbone.node_feat_dim * 2 + model.backbone.edge_feat_dim + model.backbone.time_feat_dim)
    ...
    def _obtain_train_idxs(
        ...
        elif self.dataset_name == "EffectEval":
            positive_indices = self.all_events.label == "Fortschreibung"
            pos_events = self.all_events[positive_indices].idx.values
            train_e_idxs = np.random.choice(pos_events, size=size, replace=False)
    ...
    def _train(
        ...
        for e in range(self.train_epochs):
            ...
            for i, event_idx in tqdm(
                ...
                original_pred, mask_values_ = self._tg_predict(
                    event_idx, use_explainer=False
                )
                masked_pred, mask_values = self._tg_predict(
    ...
    def explain(self, node_idx=None, event_idx=None):
        ...
        candidate_weights = {
            self.candidate_events[i]: event_idx_scores[i]
    ...
    def expose_explainer_model(
        ...
        explainer_model = PGExplainerExt._create_explainer(model, device)
