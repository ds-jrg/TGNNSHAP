...
## Model initialize
# for MODE in [ "test"]:
#     for data in ["LinkPred"]:
#         print(f"start {data} and {MODE}")
#         gnn_model_path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'params', 'tgnn', f'tgat_{data}.pt')
#         tgat = torch.load(gnn_model_path, weights_only=False)
#         tgat = tgat.to("mps")
...
#         pre_processing(tgat.ngh_finder, rand_sampler, test_src_l, test_dst_l, test_ts_l, test_e_idx_l, MODE=MODE, data=data)
