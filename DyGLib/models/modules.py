...
class MLPClassifier(nn.Module):
    def __init__(self, input_dim: int, dropout: float = 0.1, num_layers = 2, hidden_dim=32):
        """
...
class TGNNBackbone(ABC, nn.Module):
    @abstractmethod
    def __init__(self, num_nodes: int, node_dim: int, edge_dim: int,
                 time_feat_dim: int, dropout: float = 0.1, device: str = 'cpu'):
        """
        ...
        """
        super(TGNNBackbone, self).__init__()

        self.node_features = torch.zeros((num_nodes, node_dim)).to(device)
    ...
    def mask_neighbors(self, attention: torch.Tensor, neighbor_node_features: torch.Tensor) -> torch.Tensor:
        result = torch.mul(neighbor_node_features, attention.reshape(attention.shape[0],attention.shape[1], 1))
        return result
    
    def mask_timings(self, attention: torch.Tensor, edge_timing_features: torch.Tensor) -> torch.Tensor:
        result = torch.mul(edge_timing_features, attention.reshape(attention.shape[0],attention.shape[1], 1))
        return result

class TGNNModel(ABC, nn.Module):
    def __init__(self):
        super(TGNNModel, self).__init__()

    @abstractmethod
    def forward(self, src_node_embeddings:torch.Tensor, dst_node_embeddings:torch.Tensor) -> torch.Tensor:
        pass

class TGNN(nn.Module):
    def __init__(self, backbone: TGNNBackbone, model: TGNNModel):
    ...
    def forward(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                node_interact_times: np.ndarray, 
                src_subgraphs: BatchSubgraphs, dst_subgraphs: BatchSubgraphs, **kwargs):
        src_node_embeddings, dst_node_embeddings = \
            self.backbone.compute_src_dst_node_temporal_embeddings(src_node_ids=src_node_ids,
                                                                dst_node_ids=dst_node_ids,
                                                                node_interact_times=node_interact_times, 
                                                                src_subgraphs=src_subgraphs, dst_subgraphs=dst_subgraphs,
                                                                **kwargs)
        predicts = self.model(src_node_embeddings, dst_node_embeddings)
        return predicts

class NeuralNetworkDst(TGNNModel):
    def __init__(self, input_dim: int, dropout: float = 0.1, num_layers = 2, hidden_dim=32):
        """
        Multi-Layer Perceptron Classifier.
        :param input_dim: int, dimension of input
        :param dropout: float, dropout rate
        """
        super(NeuralNetworkDst, self).__init__()
        prev_size = input_dim
        layers = []
        for _ in range(num_layers-1):
            layers.append(nn.Linear(prev_size,hidden_dim))
    ...
    def forward(self, src_node_embeddings:torch.Tensor, dst_node_embeddings:torch.Tensor):
        ...
        :return:
        """
        h = self.model(dst_node_embeddings)
        return h

class NeuralNetworkSrcDst(TGNNModel):
    def __init__(self, input_dim: int, num_layers = 2, hidden_dim=32):
        """
        Multi-Layer Perceptron Classifier.
        :param input_dim: int, dimension of input
        :param dropout: float, dropout rate
        """
        super(NeuralNetworkSrcDst, self).__init__()
        prev_size = input_dim * 2
        layers = []
        for _ in range(num_layers-1):
            layers.append(nn.Linear(prev_size,hidden_dim))
            layers.append(nn.ReLU())
    ...
    def forward(self, src_node_embeddings:torch.Tensor, dst_node_embeddings:torch.Tensor):
        ...
        :return:
        """
        h = self.model(torch.hstack((src_node_embeddings, dst_node_embeddings)))
        return h
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, node_features: torch.Tensor, node_time_features: torch.Tensor, neighbor_node_features: torch.Tensor,
                neighbor_node_time_features: torch.Tensor, neighbor_node_edge_features: torch.Tensor, neighbor_masks: np.ndarray, edge_attn:Optional[torch.Tensor] = None):
        """
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inputs_query: torch.Tensor, inputs_key: Optional[torch.Tensor] = None, inputs_value: Optional[torch.Tensor] = None,
                neighbor_masks: Optional[np.ndarray] = None, edge_attn: Optional[torch.Tensor] = None):
        """
        ...
        if neighbor_masks is not None:
            ...
            neighbor_masks_tensor = torch.from_numpy(neighbor_masks).to(inputs_query.device) == 0
        else:
            neighbor_masks_tensor = None

        if edge_attn is not None:
            edge_attn = edge_attn[:,None,:].repeat((self.multi_head_attention.num_heads, inputs_query.shape[1], 1))
        ...
        hidden_states = self.multi_head_attention(query=transposed_inputs_query, key=transposed_inputs_key,
                                                  value=transposed_inputs_value, key_padding_mask=neighbor_masks_tensor, attn_mask=edge_attn)[0].transpose(0, 1)
        # Tensor, shape (batch_size, target_seq_length, self.attention_dim)
