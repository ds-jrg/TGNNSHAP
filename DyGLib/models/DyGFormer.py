...
from .modules import TimeEncoder, TGNNBackbone
from ..utils.utils import NeighborSampler, BatchSubgraphs


class DyGFormer(TGNNBackbone):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_nodes_neighbor_ids_list, src_nodes_edge_ids_list, src_nodes_neighbor_times_list, src_edge_features, src_edge_attn, src_node_attn = src_subgraphs.get_split_for_layer(0)
        ...
        dst_nodes_neighbor_ids_list, dst_nodes_edge_ids_list, dst_nodes_neighbor_times_list, dst_edge_features, dst_edge_attn, dst_node_attn = dst_subgraphs.get_split_for_layer(0)
        ...
        src_padded_nodes_edge_raw_features = torch.nn.functional.pad(src_edge_features, (0,0,1,0,0,0), value = 0)
        dst_padded_nodes_edge_raw_features = torch.nn.functional.pad(dst_edge_features, (0,0,1,0,0,0), value = 0)

        src_padded_node_attention = torch.nn.functional.pad(src_node_attn, (1,0,0,0), value=1)
        dst_padded_node_attention = torch.nn.functional.pad(dst_node_attn, (1,0,0,0), value=1)
        ...
        src_padded_nodes_neighbor_node_raw_features = self.mask_neighbors(src_padded_node_attention, src_padded_nodes_neighbor_node_raw_features)
        dst_padded_nodes_neighbor_node_raw_features = self.mask_neighbors(dst_padded_node_attention, dst_padded_nodes_neighbor_node_raw_features)
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: np.ndarray, nodes_edge_ids_list: np.ndarray,
                      nodes_neighbor_times_list: np.ndarray, patch_size: int = 1, max_input_sequence_length: int = 256):
        """
        ...
        for idx in range(nodes_neighbor_ids_list.shape[0]):
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        padded_nodes_neighbor_node_raw_features = self.node_features[torch.from_numpy(padded_nodes_neighbor_ids)]
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: torch.Tensor, padded_nodes_edge_raw_features: torch.Tensor,
                    padded_nodes_neighbor_time_features: torch.Tensor, padded_nodes_neighbor_co_occurrence_features: torch.Tensor = torch.tensor(0), patch_size: int = 1):
        """
        ...
        max_seq_length = padded_nodes_edge_raw_features.shape[1]
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        if(len(src_padded_nodes_appearances) != 0):
            ...
            src_padded_nodes_appearances = torch.tensor(src_padded_nodes_appearances, device=self.device)
        
        if(len(dst_padded_nodes_appearances) != 0):
            ...
            dst_padded_nodes_appearances = torch.tensor(dst_padded_nodes_appearances, device=self.device)
