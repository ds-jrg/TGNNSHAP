...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, edge_ids: np.ndarray, labels: np.ndarray, types: np.ndarray, dataset: Optional[pd.DataFrame] = None):
        """
...
def get_link_prediction_data(val_ratio: float, test_ratio: float, node_dim: int):
    """
    ...
    graph_df = pd.read_csv(CONFIG.data.folder + CONFIG.data.index_file)
    edge_raw_features = np.load(CONFIG.data.folder + CONFIG.data.edge_feat_file)
    ...
    assert node_dim >= node_raw_features.shape[1], f'Node feature dimension is bigger than {node_dim}!'
    # padding the features of edges and nodes to the same dimension (172 for all the datasets)
    if node_raw_features.shape[1] < node_dim:
        node_zero_padding = np.zeros((node_raw_features.shape[0], node_dim - node_raw_features.shape[1]))
    ...
    # if((graph_df.u != 0).all() and (graph_df.i != 0).all()):
    #     node_raw_features = np.concat([np.zeros((1,node_raw_features.shape[1]), dtype="float"), node_raw_features])
    ...
    labels = np.array(graph_df.label.values)
    types = np.array(graph_df.type.values)
    


    full_data = Data(src_node_ids=src_node_ids, dst_node_ids=dst_node_ids, node_interact_times=node_interact_times, edge_ids=edge_ids, labels=labels, types=types, dataset=graph_df)
    ...
    new_test_destination_mask = graph_df.i.map(lambda x: x in new_test_node_set).values.astype("bool")
    ...
    train_data = Data(src_node_ids=src_node_ids[train_mask], dst_node_ids=dst_node_ids[train_mask],
                      ...
                      edge_ids=edge_ids[train_mask], labels=labels[train_mask], types=types[train_mask])
    ...
    val_data = Data(src_node_ids=src_node_ids[val_mask], dst_node_ids=dst_node_ids[val_mask],
                    node_interact_times=node_interact_times[val_mask], edge_ids=edge_ids[val_mask], labels=labels[val_mask], types=types[val_mask])

    test_data = Data(src_node_ids=src_node_ids[test_mask], dst_node_ids=dst_node_ids[test_mask],
                     node_interact_times=node_interact_times[test_mask], edge_ids=edge_ids[test_mask], labels=labels[test_mask], types=types[test_mask])
    ...
    new_node_val_data = Data(src_node_ids=src_node_ids[new_node_val_mask], dst_node_ids=dst_node_ids[new_node_val_mask],
                             ...
                             edge_ids=edge_ids[new_node_val_mask], labels=labels[new_node_val_mask], types=types[new_node_val_mask])
    ...
    new_node_test_data = Data(src_node_ids=src_node_ids[new_node_test_mask], dst_node_ids=dst_node_ids[new_node_test_mask],
                              ...
                              edge_ids=edge_ids[new_node_test_mask], labels=labels[new_node_test_mask], types=types[new_node_test_mask])
...
def get_node_classification_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    node_interact_times = graph_df.ts.values.astype(np.float64)
    edge_ids = graph_df.idx.values.astype(np.longlong)
    labels = graph_df.label.to_numpy()
    types = graph_df.type.to_numpy()
    ...
    full_data = Data(src_node_ids=src_node_ids, dst_node_ids=dst_node_ids, node_interact_times=node_interact_times, edge_ids=edge_ids, labels=labels, types=types)
    train_data = Data(src_node_ids=src_node_ids[train_mask], dst_node_ids=dst_node_ids[train_mask],
                      node_interact_times=node_interact_times[train_mask],
                      edge_ids=edge_ids[train_mask], labels=labels[train_mask], types=types[train_mask])
    val_data = Data(src_node_ids=src_node_ids[val_mask], dst_node_ids=dst_node_ids[val_mask],
                    node_interact_times=node_interact_times[val_mask], edge_ids=edge_ids[val_mask], labels=labels[val_mask], types=types[val_mask])
    test_data = Data(src_node_ids=src_node_ids[test_mask], dst_node_ids=dst_node_ids[test_mask],
                     node_interact_times=node_interact_times[test_mask], edge_ids=edge_ids[test_mask], labels=labels[test_mask], types=types[val_mask])
