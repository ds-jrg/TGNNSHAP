...
def get_shapley_value(event_explainer:ShapleyExplainerEvents, src, dst, timestamp, event_id, sg_src=None, sg_dst=None):
    ...
    shap_value_event = shap_values[event_ids == event_id]
...
def get_shapley_value_of_masked_sg(features, event_id, event_features, event_explainer:ShapleyExplainerEvents, sg_src, sg_dst, timestamp):
    ...
    shapley = get_shapley_value(event_explainer, sg_src_pos, sg_dst_pos, timestamp, event_id)
    return shapley
...
def evaluate_file(file_name, directory, random_directory, neighbor_finder: NeighborSampler, data, event_explainer, sparsity_thresholds):
    ...
    explanations = np.load(file_path, allow_pickle=True)["coalitions"]
    top_k_events = explanations[np.sort(np.unique(explanations[:, 0], return_index=True)[0])[:k], :]
    random_explanations = np.load(random_file_path, allow_pickle=True)["coalitions"]
    top_k_random_events = random_explanations[np.sort(np.unique(random_explanations[:, 0], return_index=True)[0])[:k], :]
    
    results = []
    
    for e_id, random_e_id in zip(top_k_events[:, 0], top_k_random_events[:, 0]):
        feature_ids = explanations[explanations[:, 0] == e_id, :]
        ...
        timestamp = int(data.node_interact_times[e_id])
        ...
        for s in sparsity_thresholds:
            ...
            shapley_pos = get_shapley_value_of_masked_sg(features, e_id, event_features, event_explainer, subgraphs_src, subgraphs_dst, timestamp)
            shapley_pos_random = get_shapley_value_of_masked_sg(features_random, random_e_id, event_features, event_explainer, subgraphs_src, subgraphs_dst, timestamp)
            shapley_neg = get_shapley_value_of_masked_sg(features_neg, e_id, event_features, event_explainer, subgraphs_src, subgraphs_dst, timestamp)
            shapley_neg_random = get_shapley_value_of_masked_sg(features_random_neg, random_e_id, event_features, event_explainer, subgraphs_src, subgraphs_dst, timestamp)
...
def evaluate_feature_explanations(model, sampler, data, event_features, sparsity_thresholds) -> None:
    ...
    random_explainer_name = "random_feature"
